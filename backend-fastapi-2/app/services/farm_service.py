...
async def create_cow(conn, farmer_id: str, name: str, age: int, status: int):
    query = """
        INSERT INTO farm.cow (farmer_id, name, age, status)
        VALUES ($1, $2, $3, $4)
        RETURNING cow_id, farmer_id, name, age, status
    """
    record = await conn.fetchrow(query, farmer_id, name, age, status)
    return dict(record)

async def get_cows(conn, farmer_id: str):
    query = "SELECT * FROM farm.cow WHERE farmer_id = $1"
    rows = await conn.fetch(query, farmer_id)
    return [dict(r) for r in rows]

async def get_cow_by_id(conn, cow_id: str):
    query = "SELECT * FROM farm.cow WHERE cow_id = $1"
    record = await conn.fetchrow(query, cow_id)
    if not record:
        raise HTTPException(status_code=404, detail="Cow not found")
    return dict(record)

async def update_cow(conn, cow_id: str, name: str | None = None, age: int | None = None, status: int | None = None):
    query = "UPDATE farm.cow SET name = COALESCE($2, name), age = COALESCE($3, age), status = COALESCE($4, status) WHERE cow_id = $1 RETURNING *"
    record = await conn.fetchrow(query, cow_id, name, age, status)
    if not record:
        raise HTTPException(status_code=404, detail="Cow not found")
    return dict(record)
