...
async def get_active_cow_by_rfid(db: asyncpg.Connection, rfid_id: str) -> UUID | None:
    """
    Memvalidasi bahwa RFID terdaftar dan DITUGASKAN ke seekor sapi.
    Mengembalikan cow_id jika valid, jika tidak None.
    """
    if not rfid_id:
        return None
...
async def create_eat_session(
    ...
    """
    Memasukkan eat_session yang sudah selesai ke database.
    """
    # Validasi data: jangan simpan sesi jika berat tidak berkurang
    ...
    try:
        ...
        print(f"(SESSION CREATED) Cow {cow_id} at {device_id} finished.")
